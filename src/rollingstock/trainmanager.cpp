...
void TrainManager::inittrain(State startstate)
{
	...
	startstate = Tracks::travel(*tracks, startstate, -53/2);
	startstate = Tracks::travel(*tracks, startstate, 49/2);
	for(int iWagon=0; iWagon<2; iWagon++){
		startstate = Tracks::travel(*tracks, startstate, -(49));
		Wagon* newwagon = new Wagon(tracks, startstate, rollingstock.gettypefromid(WagonID::openwagon));
		addwagon(newwagon);
