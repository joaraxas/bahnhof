...
// Handles the pool of available control modes to allow a player to switch 
// between roles of action.
class ControlManager
{
public:
    ControlManager(Game* whatgame);
    const Economy::ControlMode& getcontrolmode() const;
    const std::vector<Economy::ControlMode>& getavailablecontrolmodes() const{
        return controlmodes;
    };
    void addcontrolmode(Economy::ControlMode mode) {
